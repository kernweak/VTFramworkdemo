                        ...
                        )
{
    ...
    *(ULONG64 *) pCpu->OriginalVmcs  = (__readmsr(MSR_IA32_VMX_BASIC) & 0xffffffff); 
    ...
    if (__vmx_on (&PhyAddr))//建立了特权层host
    {
...
HvmSpitOutBluepill ()
{
    ...
    for (i = 0; i < KeNumberProcessors; i++)
    {
        ...
        VmxVmCall (NBP_HYPERCALL_UNLOAD);//vmx-asm.asm,主动引发一个vmcall 的exit事件
		FreeCpuMem();
...
HvmSwallowBluepill ()
{
    ...
    for (i = 0; i < KeNumberProcessors; i++)
    {
        ...
        Status = CmSubvert (NULL);  // common-asm.asm CmSubvert的流程是保存所有寄存器(除了段寄存器)的内容到栈里后，调用HvmSubvertCpu
