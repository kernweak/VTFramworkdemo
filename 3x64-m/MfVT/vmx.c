...
BOOLEAN VmxIsImplemented ()
{
    ...
    GetCpuIdInfo (0x1, &eax, &ebx, &ecx, &edx);//common-asm.asm
    return (ecx & (1<<5));
}
...
)
{
	...
	__vmx_vmread(VM_EXIT_REASON, &ExitReason); //原因
    __vmx_vmread(GUEST_RIP, &GuestEIP);//GuestEIP +inst_len=guest下一条要执行的指令的地址
    __vmx_vmread(VM_EXIT_INSTRUCTION_LEN, &inst_len);//拿到长度为了计算下一条指令地址，为了resume
    ...
    else if (ExitReason == EXIT_REASON_MSR_READ)  // done!
    {
        ...
        switch (ecx)
        {
        case MSR_LSTAR://patchguard就是定期读取MSR内容，就是syscall地址，我们可以让他进入我们自己的流程，不给我们修改的地址，返回原来的值
    ...
    __vmx_vmwrite(GUEST_RIP, GuestEIP + inst_len);
	//在vmx-asm.asm后面继续调用vmresume()进入guest
}
...
NTSTATUS VmxSetupVMCS (
  ULONG_PTR VMM_Stack,//host rsp
  PVOID GuestRip,//guest rip
  PVOID GuestRsp //guest rsp
)
{
...
////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  /////////////////////////////////////////////////////////////////////////////
  __vmx_vmwrite (GUEST_RSP, (ULONG64) GuestRsp);     //setup guest sp，结构体，guest所有寄存器
  __vmx_vmwrite (GUEST_RIP, (ULONG64) GuestRip);     //setup guest ip : common-asm CmResumeGuest
  __vmx_vmwrite (GUEST_RFLAGS, RegGetRflags ());	 //标志寄存器,因为从host到guest需要这三个恢复上下文

  // HOST_RSP与HOST_RIP决定进入VMM的地址
  __vmx_vmwrite (HOST_RSP, VMM_Stack);
  __vmx_vmwrite (HOST_RIP, (ULONG64) VmxVmexitHandler);//vmx-asm.asm-->VmexitHandler-->vmx.c
...
)
{
  ...
  // 下面的代码：construct stack frame for IRETQ:
  // push ss
  // push rsp
  // push rflags
  // push cs
  // push rip guest回到真实CPU上之后，下一条要执行的指令
  // ret
...
{
  UCHAR Trampoline[0x600];//存放着机器码，用来负责把guest中的寄存器和rip 恢复到真实场景中来
